...
b. & {I wish I had a car.} \\
   & {(conveying ``I don't have a car now.'' \citep[p.\ 231--232]{iatridou:00})} \\
\end{tabular}}}
...
lexical markers shows up in \ili{Japanese} and \ili{Korean}. As is
well-known, \wa in Japanese and \nun in Korean are regarded as lexical
markers to express \isi{topic}, but they can also sometimes be used for
conveying \isi{contrastive focus}.
...
This \nun-marked constituent associated with \isi{contrastive focus} is
realized differently from one associated with \isi{contrastive topic}. In
...
elements with contrastive meaning can be scrambled in \ili{Korean},
which means \textit{ku chack-un} `the book-\textsc{nun}' in
(\ref{exe:kor:ct}A) gives contrastive meaning.\is{contrast}
...
\nun-marked constituents are analyzed as only contrastive \isi{focus} in her
...
So, \nun in \ili{Korean} can assign three meanings to an adjoining NP:
aboutness topic, contrastive \isi{topic}, and contrastive \isi{focus}.  In other
...
ambiguous interpretation, either focus or \isi{background}
(i.e.\ non-topic).



In some languages, a lexical marker known for marking \isi{topic} coincides
with cleft constructions which clearly carry a \isi{focus}
meaning.\is{clefting} \myref{exe:hil} in \ili{Ilonggo}
...
in \ili{Korean}. One difference is that the case relation is marked by
...
{`It was the woman who brought a child.' [hil] \citep[p.\ 108]{croft:02}}}}
...
\\
\multicolumn{10}{r}{ [inh] \citep[p.\ 683]{nichols:11}}\\
\end{tabular}}}}


\noindent That means that if \isi{topic} and (narrow) \isi{focus} co-occur, topic
should be followed by focus even in languages which place focused
constituents in the clause-initial position.\is{clause-initial}\is{narrow focus}
The same phenomenon can be found in many other languages.  For example, in
\ili{Nishnaabemwin} (an Algic language spoken in the region surrounding the
...
hitherto.\is{preverbal}




Yet, there are some cases in which it is unclear which role
(i.e.\ \isi{focus} or topic) the fronted constituent is assigned to.  I
would like to call the constructions in which this kind of ambiguity
takes place `focus/\isi{topic} \isi{fronting}' (a.k.a.\ Topicalization).\is{topicalization}
\citet{prince:84} provides two types of OSV constructions in \ili{English},
...
\item{Mary, John saw her yesterday. \citep[p.\ 213]{prince:84}}}}
...
\citeauthor{prince:84},\footnote{\citet{prince:84} argues that the
  ...
  that disfavours it.\is{syntactic positioning}
  As indicated in Chapter~\ref{chapter3} (\myS{3:sec:status}), since
  ...
  used in the present work.} but I use the label \isi{focus}/\isi{topic} \isi{fronting}
...
still lead to confusion.\footnote{From a different point of view, some
  \ili{English} native speakers say that (\ref{exe:topicalization}c) does
  not look like a proper paraphrasing of
  (\ref{exe:topicalization}a).\is{clefting} In the intuition, the
  fronted item \textit{The book} conveys only \isi{focus} meaning.  If his
  thought holds true, the focus/\isi{topic} \isi{fronting} constructions are
...
{(a) `It is this book that I like.' or \\(b) `As for this book, I like it.' [yue] \citep[p.\ 16]{man:07}}}}
...
in \ili{English} and \ili{Cantonese}, becomes ambiguous if only one
argument is \isi{preverbal} \citep{valentine:01}.  In fact, this kind
of ambiguity frequently happens in languages in which \isi{focus} shows up
clause-initially (e.g.\ \ili{Ingush},
\citep{nichols:11}).\is{clause-initial}
...
\citet{buring:10} calls the third type `Mixed Languages' and creates
...
that necessary to \isi{focus} X in the unmarked
    constituent order. \citep[p.\ 197]{buring:10}}}
...
clearly shown with the \textit{wh}-test.\is{\textit{wh}-test} If the question is 
...
Even if a language employs a specific position for expressing \isi{focus},
the focused constituent does not necessarily take that position, as
exemplified in \ili{Russian} in the previous section. That is, focus
can be assigned to multiple positions.\is{clause-final}  For instance, the focus in
Russian may not be clause-final (as presented in \myref{exe:sobaka}),
if the accent falls on another constituent. In this case, clause-final
focus does not seem to be the same as cleft constructions in Russian,\is{clefting}
...
respect to syntactic operations on focus is exemplified by \ili{Greek}
\citep{gryllia:09}. In Greek, whose basic word order is VSO or SVO,
focus can be both \isi{preverbal} and \isi{postverbal}
...
\wa and \nun in \ili{Japanese} and \ili{Korean} respectively are \isi{topic}
markers in these languages, but they can sometimes be used for
expressing \isi{contrastive focus}. Second, topic and focus appear
...
these constructions as examples of focus/topic \isi{fronting}. Third,
...
structure in unmarked sentences is addressed in \myS{9:ssec:underspecification} \mypage{9:ssec:underspecification}
