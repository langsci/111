...
\myS{8:sec:hpsg} investigates HPSG-based studies on information
structure, which are largely based on a pioneering study offered by
\citet{engdahl:vallduvi:96}.  \myS{8:sec:mrs} looks into how several
...
with information structure, \myS{8:sec:phonology} surveys related
works.  \myS{8:sec:other-frameworks} offers an explanation of how
...
MRS,\is{MRS} but the values in the lists of components of information
structure are co-indexed with the value of INDEX (e.g.\ \tdl{x1},
\tdl{e2}, etc.).  These two methods represent just two of many methods
for representing information structure in HPSG and
MRS.\is{HPSG}\is{MRS} Taking a different approach, \citet{chang:02}
represents information structure using just a string (as presented in
\myref{avm:chang:tf} \mypage{avm:chang:tf}). \citet{kim:07} and
...
(as shown in \myref{avm:kim:beer} \mypage{avm:kim:beer}).  Sometimes,
...
components in \citet{engdahl:vallduvi:96}'s proposal; \isi{focus} and
\isi{ground}. While ground acts as an usher for focus, focus is
...
state.\footnote{Note that ground is not the same as
  \isi{background}. Ground is thought of as opposite to focus, while
  background is neither focus nor \isi{topic}. Tail, so to speak, has
...
sentential focus (a.k.a.\ \tdl{all-focus} in \citet{paggio:09} and the
present study), such as a reply to questions like \textit{What
  happened?}, are not required to include ground.\is{background} Since
...
A1: & {Yes. The \textbf{president} [$_{f}$ hates the
  ...
  it.) \citep[p.\ 5]{engdahl:vallduvi:96}}\\
\end{tabular}}}
...
for link, and \tdl{bg} (\isi{background}) for tail). The type
hierarchy \citeauthor{paggio:09} proposes for \ili{Danish} is shown in
Figure~\ref{fig:hier:paggio} (Ibid.\ p.\ 140), and the lowest subtypes
...
\item\shortex{6} {(Hvad & lavede & b{\o}rnene?) & [$_{T}$ De] &
  [$_{F}$ spiste & is].}  {(what & did & children.\textsc{def}) & they
  & ate & icecream} {`What did the children do? They ate icecream.'
  (\tdl{topic-focus})}
\item\shortex{6} {(Hvad & spiste & b{\o}rnene?) & [$_{BG}$ [$_{T}$ De]
    & spiste] & [$_{F}$ is].}  {(what & ate & children.\textsc{def}) &
  they & ate & icecream} {`What did the children eat? They ate
  icecream.' (\tdl{topic-focus-bg})}
\item\shortex{7} {(Hvem & har & spist & isen?) & [$_{BG}$ Det & har] &
  [$_{F}$ b{\o}rnene].}  {(who & has & eaten & icecream.\textsc{def})
  & that & have & children.\textsc{def}} {`Who has eaten the icecream?
  The children did.' (\tdl{bg-focus})}
\item\shortex{6} {(Hvad & skete & der?) & [$_{F}$ B{\o}rnene & spiste]
  & is].}  {(what & happened & there) & children.\textsc{def} & ate &
    icecream} {`What happened? The children ate icecream.'
    (\tdl{all-focus}) [dan] \citep[p.\ 139]{paggio:09}}}}



The present study agrees that information structure needs to be
...
\citep[p.\ 122]{lambrecht:96}.\footnote{The \tdl{bg-focus} sentential
  form is similar to cleft constructions.\is{clefting}} For this
...
question for future work.\is{HPSG} My understanding is that the most
...
that using \isi{underspecification} is a more effective way to represent
information structure; especially, for the purpose of implementing
HPSG-based NLP applications (e.g.\ machine translation, \isi{TTS}
...
variants of \myref{avm:engdahl:vallduvi:words} in \ili{English}.
...
introducing a zero \isi{topic} \nun, a thematic topic \nun, and a
\isi{contrastive topic} \nun are three separate homonyms.  The use of
...
\myref{avm:kim:beer}. Since \textit{beer} in (\ref{exe:kim:beer}A) is
contrastively focused (i.e.\ an answer to an alternative question
\citep{gryllia:09}),\is{contrastive focuse} it has both [HIGHLIGHT +] and [FOCUS +] in his
analysis. Note that [HIGHLIGHT \tdl{bool}] in \myref{avm:kim:beer}
indicates whether or not the constituent conveys a contrastive
meaning, which is almost the same as [CONTRAST \tdl{bool}].


\myexe{\enumsentence{\label{exe:kim:beer}
\begin{tabular}[t]{ll}
Q: & {Did John drink beer or coke?}\\ 
A: & {John drank beer. \citep[p.\ 229]{kim:07}}\\
\end{tabular}}}
...
representation via \isi{ICONS} (Individual CONStraints).\is{individual
  constraints} The main motivation for \isi{ICONS} is the ability to
...
provides fundamentals of \isi{ICONS} in detail.
...
(especially, semantic \isi{focus} \citep{gundel:99}) is sometimes relevant
to \isi{truth-conditions} and scopal interpretation (i.e.,\ topics
...
for representing information structure, modeling the scope of \isi{focus}
analogously to \isi{quantifier} scope (i.e.\ HCONS).\is{MRS}
...
binary relations in the same way as HCONS is.\is{binary relation}
...
the wide \isi{focus} reading of (\ref{exe:wilcock}a) (i.e.\ from 3 to
5). Note that in this representation, LINK (\tdl{topic} in this paper)
and FOCUS have no relation to the clause or its head (\textit{hate}).\is{topic}





\citet{yoshimoto:etal:06} uses MRS,\is{MRS} too. In their model,
...
\isi{\mbox{infrastructure}} which requires all transfer-related ingredients
...
\citep{bolinger:61,jackendoff:72}).\is{B-accent}
...
\tdl{pwrd}, whose relationship is ruled as follows
...
hierarchy of \tdl{tone} and the typed feature structure for
...
The \isi{CCG} (Combinatory Categorial Grammar,
...
\citep{steedman:00}.\footnote{CCG departing from CC (Categorial
  ...
  HPSG-style typed feature structures (i.e.\ \tdl{sign}). The
...
\myS{12:sec:machinery} \mypage{12:sec:machinery}).  Though I am not
...
discussed in \myS{8:sec:phonology},\is{prosody} but future work will
...
sketched out in \myref{fig:choi} (Ibid.\ p.\ 92).\footnote{In
...
structure.  This is discussed in more detail in \myS{9:ssec:binary}
and \myS{10:ssec:relative:background}.  Second, LFG-based studies deal
